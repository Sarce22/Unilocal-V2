...
object Lugares {
    ...
    init {
        ...
        val lugar1 = Lugar("La sabrosura del mar", "La mejor comida que podrás encontrar", 1, EstadoLugar.SIN_REVISAR, 3, "Calle 123", 1)
        lugar1.id = 1
        lugar1.horarios.add(horario2)

        val lugar2 = Lugar("Arepas la mona", "Mejor lugar para desayunar", 2, EstadoLugar.ACEPTADO, 3, "Calle 12 # 23-1", 1)
        lugar2.id = 2
        lugar2.telefonos = tels
        lugar2.horarios.add(horario1)

        val lugar3 = Lugar("Perros el pri", "Comida rapida para todos", 3, EstadoLugar.SIN_REVISAR, 3, "Calle 452", 5)
        lugar3.id = 3
        lugar3.horarios.add(horario1)

        val lugar4 = Lugar("La empanada #1", "Las mejores empanadas", 3, EstadoLugar.ACEPTADO, 5, "Calle 53",3)
        lugar4.id = 4
        lugar4.horarios.add(horario3)

        val lugar5 = Lugar("Hotel marroco", "Hotel para ahorrar dinero", 1, EstadoLugar.ACEPTADO, 1, "Calle 23 # 34-1", 1)
        lugar5.id = 5
        lugar5.horarios.add( horario1 )

        val lugar6 = Lugar("Alojamientos", "Alojamiento para todos y todas", 1, EstadoLugar.SIN_REVISAR, 1, "Carrera 123", 2)
        lugar6.id = 6
